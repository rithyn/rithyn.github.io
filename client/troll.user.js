...
console.clear()
document.open();
document.write(`<html><head><meta charset="UTF-8"><title>Troll Client</title><style>body{background:#000}body,div,html{margin:0;padding:0;color:#fff;font-family:Tahoma,Verdana}div{font-size:2em}span{margin:50px}#content{overflow:hidden;position:absolute;left:0;right:0;bottom:0;top:0;opacity:0;transition:all 3s;z-index:0}#logo{overflow:hidden;text-align:center;position:fixed;left:0;right:0;bottom:0;top:0;opacity:1;transition:all 3s;z-index:2}#logo img{transition:opacity 1s;opacity:0;width:50%}#neuronality{position:fixed;bottom:8vh;left:0;right:0;text-align:center;transition:all 3s;z-index:1;color:#ffea08;font-size:4em}iframe{border:none}</style></head><body><div id="logo"><img src="https://raw.githubusercontent.com/officialtroller/starblast-things/main/clientlolgo.png"></div><div id="neuronality">Please Wait ...</div><script>var logo=document.querySelector("#logo img");logo.onload=function(){logo.style.marginTop=(.8*window.innerHeight-logo.offsetHeight)/2+"px",logo.style.opacity="1"}</script><div id="content"></body></html>`);
document.close();
log(`Started`)
